<x-app-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div>
                        ...
                        <p class="mt-2 text-gray-600 max-w-lg">
                            Ini adalah pusat kendali Anda. Pantau statistik, kelola pasien, dan catat perkembangan rehabilitasi dengan mudah.
                        </p>
                    </div>
                    <div class="shrink-0">
                        {{-- Logo LRPPN --}}
            ...
            {{-- Layout Grid Utama (Untuk Statistik & Sidebar) --}}
                        ...
                        <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg p-6 flex items-center space-x-4
                            ...
                            <div class="shrink-0 bg-indigo-100 rounded-full p-4">
                                <svg class="h-8 w-8 text-indigo-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                  <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3.004 3.004 0 013.75-2.906z" />
                                </svg>
                            </div>
                            <div>
                                <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider">Total Pasien</h4>
                                <p class="text-2xl font-bold text-gray-900 mt-1">{{ $totalPasien }}</p>
                            </div>
                        </div>
                        
                        {{-- Kartu 2: Klasifikasi Bulan Ini --}}
                        <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg p-6 flex items-center space-x-4
                            ...
                            <div>
                                <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider">Klasifikasi Baru</h4>
                                <p class="text-2xl font-bold text-gray-900 mt-1">{{ $klasifikasiBulanIni }}</p>
                                <p class="text-xs text-gray-400">Bulan Ini</p>
                            </div>
                        </div>
                        
                        {{-- Kartu 3: Catatan SOAP Hari Ini --}}
                        <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg p-6 flex items-center space-x-4
                                    transform transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                            <div class="shrink-0 bg-blue-100 rounded-full p-4">
                                <svg class="h-8 w-8 text-blue-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                  <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm2 4a1 1 0 011-1h6a1 1 0 110 2H7a1 1 0 01-1-1zm1 3a1 1 0 100 2h6a1 1 0 100-2H7z" clip-rule="evenodd" />
                                </svg>
                            </div>
                            <div>
                                <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider">Catatan SOAP</h4>
                                <p class="text-2xl font-bold text-gray-900 mt-1">{{ $soapHariIni }}</p>
                        ...
                        <ul role="list" class="divide-y divide-gray-100">
                            @forelse ($pasienTerbaru as $pasien)
                                <li class="transition-colors duration-150 hover:bg-slate-50">
                                    <a href="{{ route('admin.pasien.show', $pasien->id) }}" class="flex items-center justify-between px-6 py-4">
                                        <div class="flex items-center space-x-4">
                                            <div class="shrink-0 h-10 w-10 flex items-center justify-center bg-indigo-100 rounded-full text-indigo-600 font-bold text-sm">
                                                {{ substr($pasien->name, 0, 2) }}
                                            </div>
                                            <div>
                                                <p class="text-sm font-semibold text-gray-900">{{ $pasien->name }}</p>
                                                <p class="text-xs text-gray-500">{{ $pasien->email }}</p>
                                            </div>
                                        </div>
                                        <div class="flex items-center text-xs text-gray-400">
                                            {{ $pasien->created_at->diffForHumans() }}
                                            <svg class="h-4 w-4 ml-2 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
